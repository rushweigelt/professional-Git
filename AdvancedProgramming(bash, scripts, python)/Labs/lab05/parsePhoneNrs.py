...
def searchFile( fileName, pattern ) :
	...
	for l in fh :
		...
		if match :
			...
			#CHANGES: this if else statement adds needed print statements to accept possible extensions
			if not nr[3]:
				 print "area code: " + aCode + \
					", exchange: " + nr[1] + ", trunk: " + nr[2]
			else:
...
def main() :
	...
	patString = r'''
		...
		[- .]*			# optional separator (any # of space, dash, or dot)
		(\d{4})			# number, 4 digits
		[ \-x]*			#CHANGE: optional seperator found by searching for the optional x char
		(\d{1,4})?		#CHANGE: num between 1-4 that are our possible extensions
		'''
